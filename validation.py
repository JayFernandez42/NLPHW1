...
def evaluate_model(model, val_set, tags):
  ...
  for tokens, labels, indices in zip(val_set['text'], val_set['NER'], val_set['index']):
    predicted_tags = viterbi(model, tokens, tags)
    y_pred_dicts.append(format_output_labels(predicted_tags, indices))
    y_true_dicts.append(format_output_labels(labels, indices))
  
  f1_scores = [mean_f1(y_pred, y_true) for y_pred, y_true in zip(y_pred_dicts, y_true_dicts)]
