# Name(s): Grant Zhao, Jacob Fernandez
# Netid(s): gz233, jaf388
################################################################################
...
def handle_unknown_words(t, documents): 
    ...
    """
    token_counts = Counter(token for doc in documents for token in doc)
    
    sorted_tokens = sorted(token_counts.items(), key=lambda x: (x[1], x[0]))
    
    num_replace = int(t * len(sorted_tokens))
    
    tokens_to_replace = {token for token, _ in sorted_tokens[:num_replace]}
    
    new_documents = [
        [token if token not in tokens_to_replace else "<unk>" for token in doc]
        for doc in documents
    ]
    
    vocab = sorted(set(token_counts.keys()) - tokens_to_replace) + ["<unk>"]
...
def apply_smoothing(k, observation_counts, unique_obs):
    ...
    smoothed_probs = {}
    vocab_size = len(unique_obs)

    for (state, obs), count in observation_counts.items():
        smoothed_prob = (count + k) / (state_counts[state] + k * vocab_size)

        smoothed_probs[(state, obs)] = math.log(smoothed_prob)
